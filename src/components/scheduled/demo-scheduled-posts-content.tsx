...
import { Pencil, CalendarClock, Trash2, Plus } from "lucide-react"
...
/** Demo Mode Scheduled Posts - same UI as the real page, backed by the
 * demo posts store. Edit/reschedule/delete mutate demo state only. */
export function DemoScheduledPostsContent() {
  ...
  return (
    <div className="flex flex-1 flex-col gap-5 px-4 py-6 sm:px-6 lg:px-8 animate-in fade-in-0 slide-in-from-bottom-2 duration-500 ease-out">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">
            {base.filter((p) => p.status === "scheduled").length} scheduled · {base.filter((p) => p.status === "failed").length} failed · Demo workspace
          </p>
        </div>
      ...
      <EditPostDialog
        ...
        onSave={(id, patch) => updatePost(id, patch)}
      />
      <ScheduleDialog
        ...
        onConfirm={(iso) => {
          if (reschedulingPost) {
            updatePost(reschedulingPost.id, { status: "scheduled", scheduledFor: iso })
          }
